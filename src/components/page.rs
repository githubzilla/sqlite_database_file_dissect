...
impl Page {
    pub fn try_from_be_bytes(bytes: &[u8], header_start_index: Option<usize>) -> Result<Self, MyError> {

        //page header
        let (header, header_start_idx) = match header_start_index {
            Some(start_idx) => (PageHeader::try_from_be_bytes(&bytes[start_idx..]).unwrap(), start_idx),
        ...
        let cell_pointers_length: usize = (std::mem::size_of::<u16>() as usize * cell_number).into();
